...
namespace glutil
{
	...
	void MatrixStack::PixelPerfectOrtho(glm::ivec2 size, glm::vec2 depthRange, bool isTopLeft)
	{
		if (isTopLeft)
		{
			Translate(glm::vec3(-1.0f, 1.0f, (depthRange.x + depthRange.y) / 2.0f));
			Scale(glm::vec3(2.0f / size.x, -2.0f / size.y, 1.0f));
		}
		else
		{
			Translate(glm::vec3(-1.0f, -1.0f, (depthRange.x + depthRange.y) / 2.0f));
			Scale(glm::vec3(2.0f / size.x, 2.0f / size.y, 2.0f / (depthRange.y - depthRange.x)));
		}
	}
